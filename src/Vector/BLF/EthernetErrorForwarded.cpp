...
std::vector<uint8_t>::iterator EthernetErrorForwarded::fromData(std::vector<uint8_t>::iterator it) {
...
void EthernetErrorForwarded::toData(std::vector<uint8_t> & data) {
    /* pre processing */
...
WORD EthernetErrorForwarded::calculateStructLength() const {
    return
        sizeof(flags) +
        sizeof(channel) +
        sizeof(hardwareChannel) +
