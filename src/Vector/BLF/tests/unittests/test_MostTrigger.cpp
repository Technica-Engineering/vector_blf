...
/* MOST_TRIGGER = 39 */
BOOST_AUTO_TEST_CASE(MostTrigger_) {
    Vector::BLF::File file;
    file.open(CMAKE_CURRENT_SOURCE_DIR "/events_from_binlog/test_MostTrigger.blf");
    ...
    BOOST_CHECK_EQUAL(obj->reservedMostTrigger, 0x2222);
    BOOST_CHECK_EQUAL(obj->mode, 0x3333);
    BOOST_CHECK_EQUAL(obj->hw, 0x4444);
    BOOST_CHECK_EQUAL(obj->previousTriggerValue, 0x55555555);
    BOOST_CHECK_EQUAL(obj->currentTriggerValue, 0x66666666);
...
BOOST_AUTO_TEST_CASE(MostTrigger_2) {
    Vector::BLF::File file;
    file.open(CMAKE_CURRENT_SOURCE_DIR "/events_from_converter/test_MostTrigger.blf");
    BOOST_REQUIRE(file.is_open());
