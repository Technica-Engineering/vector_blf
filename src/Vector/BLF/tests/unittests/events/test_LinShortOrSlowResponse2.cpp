...
BOOST_AUTO_TEST_CASE(LinShortOrSlowResponse2)
{
    ...
    file.open(CMAKE_CURRENT_SOURCE_DIR "/events/test_LinShortOrSlowResponse2.blf");
    BOOST_REQUIRE(file.is_open());

    Vector::BLF::ObjectHeaderBase * ohb = file.read();
    BOOST_REQUIRE(ohb != nullptr);
    BOOST_REQUIRE(ohb->objectType == Vector::BLF::ObjectType::LIN_SHORT_OR_SLOW_RESPONSE2);
    Vector::BLF::LinShortOrSlowResponse2 * obj = static_cast<Vector::BLF::LinShortOrSlowResponse2 *>(ohb);
